#!/bin/sh
sudo mkdir /usr/src/mk_arcade_joystick_rpi-0.1.5.5
sudo cp -a * /usr/src/mk_arcade_joystick_rpi-0.1.5.5/
sudo apt-get install -y --force-yes dkms cpp-4.7 gcc-4.7 joystick raspberrypi-kernel-headers wiringpi
sudo dkms build -m mk_arcade_joystick_rpi -v 0.1.5.5
sudo dkms install -m mk_arcade_joystick_rpi -v 0.1.5.5
