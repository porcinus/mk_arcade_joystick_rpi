...
module_param_array_named ( gpio, gpio_cfg.mk_arcade_gpio_maps_custom, int, & ( gpio_cfg.nargs ),
                           0 );
...
module_param_array_named ( gpio2, gpio_cfg2.mk_arcade_gpio_maps_custom, int, & ( gpio_cfg2.nargs ),
                           0 );
...
module_param_array_named ( auto_center_analog, auto_center_cfg.auto_center, int,
                           & ( auto_center_cfg.nargs ), 0 );
...
module_param_array_named ( x1dir, analog_x1_direction_cfg.dir, int,
                           & ( analog_x1_direction_cfg.nargs ), 0 );
MODULE_PARM_DESC ( x1dir, "X1 analog direction" );

module_param_array_named ( y1dir, analog_y1_direction_cfg.dir, int,
                           & ( analog_y1_direction_cfg.nargs ), 0 );
MODULE_PARM_DESC ( y1dir, "Y1 analog direction" );

module_param_array_named ( x2dir, analog_x2_direction_cfg.dir, int,
                           & ( analog_x2_direction_cfg.nargs ), 0 );
MODULE_PARM_DESC ( x2dir, "X2 analog direction" );

module_param_array_named ( y2dir, analog_y2_direction_cfg.dir, int,
                           & ( analog_y2_direction_cfg.nargs ), 0 );
...
module_param_array_named ( x1params, analog_x1_abs_params_cfg.abs_params, int,
                           & ( analog_x1_abs_params_cfg.nargs ), 0 );
MODULE_PARM_DESC ( x1params, "X1 ADC absolute parameters (min,max,fuzz,flat)" );

module_param_array_named ( y1params, analog_y1_abs_params_cfg.abs_params, int,
                           & ( analog_y1_abs_params_cfg.nargs ), 0 );
MODULE_PARM_DESC ( y1params, "Y1 ADC absolute parameters (min,max,fuzz,flat)" );

module_param_array_named ( x2params, analog_x2_abs_params_cfg.abs_params, int,
                           & ( analog_x2_abs_params_cfg.nargs ), 0 );
MODULE_PARM_DESC ( x2params, "X2 ADC absolute parameters (min,max,fuzz,flat)" );

module_param_array_named ( y2params, analog_y2_abs_params_cfg.abs_params, int,
                           & ( analog_y2_abs_params_cfg.nargs ), 0 );
...
struct i2c_client *i2c_new_ADS1015 ( struct i2c_adapter *adapter,
                                     u16 address ) { //nns: add ads1015 support
...
static int16_t ADC_OffsetCenter ( uint16_t adc_resolution, uint16_t adc_value, uint16_t adc_min,
                                  uint16_t adc_max, int16_t adc_offset ) {
	int16_t adc_center;
	int16_t range;
	int32_t ratio;
	int16_t corrected_value; //used variables
	...
	if ( adc_value < ( adc_center + adc_offset ) ) { //value under center offset
		...
		if ( range != 0 ) { //to avoid divide by 0
			...
			corrected_value = adc_value;   //range=0, setting problems?
		}
	...
	if ( corrected_value < 1 ) {
		corrected_value = 1;   //constrain computed value to 12bits value + fix for Reicast overflow
	} else if ( corrected_value > 4094 ) {
		corrected_value = 4094;
	}
...
static int16_t ADC_Deadzone ( uint16_t adc_value, uint16_t min, uint16_t max, uint16_t flat ) {
	...
	if ( adc_value > adc_center - flat && adc_value < adc_center + flat ) {
		adc_value = adc_center;   //apply flat value to adc value
	}

	return adc_value;
}

static int16_t ADS1015_read ( const struct i2c_client *client,
                              uint16_t axis ) { //based on https://github.com/torvalds/linux/blob/master/drivers/hwmon/ads1015.c
	int16_t value = 0; //used variables
	int16_t ain = ads1015_lookup[axis]; //get ain id

	if ( ain < 0 || ain > 3 ) {
		return -1;   //fail: ain oob, return -1
	}

	i2c_smbus_write_word_swapped ( client, 0x01,
	                               0x83E3 | ads1015_ain[ain] ); //default config but +-4.096v FSR and 3300SPS
	...
	if ( value >= 0 ) { //success
		...
		return value;   //fail: return i2c error
	}
}
		...
		if ( pad->gpio_maps[i] != -1 ) { // to avoid unused buttons
			if ( ( i == 12 ) && ( pad->hotkey_mode == HOTKEY_MODE_TOGGLE ) ) { //the hotkey
				...
				if ( hk_state != hk_state_prev ) { //the hotkey changed
					...
					} else if ( hotkey_combo_btn ==
					            i ) { //the hotkey was just released, and the PWR btn itself is the hotkey
...
static void mk_input_report ( struct mk_pad *pad, unsigned char *data ) {
	...
	if ( x1_enable ) { //if using analog
		...
		input_report_abs ( dev, ABS_X, !data[2] - !data[3] );   //DPAD is ABS_X
	}

	if ( y1_enable ) { //if using analog
		input_report_abs ( dev, ABS_HAT0Y, !data[0] - !data[1] ); //DPAD is ABS_HAT0Y
	} else {
		input_report_abs ( dev, ABS_Y, !data[0] - !data[1] );   //DPAD is ABS_Y
	}

	if ( x1_enable ) {
		if ( ads1015_enable ) {
			adc_val = ADS1015_read ( i2c_client_x1, 0 ); //ads1015
		} else {
			adc_val = i2c_smbus_read_word_swapped ( i2c_client_x1, 0 );   //mcp3021
		}

		if ( adc_val >= 0 ) {
			if ( x1_reverse ) {
				adc_val = 4096 - adc_val;   //nns: reverse 12bits value
			}

			if ( adc_val < x1_min ) {
				x1_min = adc_val;   //update x1 analog min value
			}

			if ( adc_val > x1_max ) {
				x1_max = adc_val;   //update x1 analog max value
			}

			if ( auto_center ) { //nns: adc value correction
				adc_val = ADC_OffsetCenter ( 4096, adc_val, x1_analog_abs_params.min, x1_analog_abs_params.max,
				                             x1_offset ); //re-center adc value
				adc_val = ADC_Deadzone ( adc_val, 0x000, 0xFFF,
				                         x1_analog_abs_params.flat ); //apply flat value to adc value
			} else {
				adc_val = ADC_Deadzone ( adc_val, x1_analog_abs_params.min, x1_analog_abs_params.max,
				                         x1_analog_abs_params.flat );   //apply flat value to adc value using min and max value of the axis
			}
	...
	if ( y1_enable ) {
		if ( ads1015_enable ) {
			adc_val = ADS1015_read ( i2c_client_x1, 1 ); //ads1015
		} else {
			adc_val = i2c_smbus_read_word_swapped ( i2c_client_y1, 0 );   //mcp3021
		}
		...
		if ( adc_val >= 0 ) {
			...
			if ( adc_val < y1_min ) {
				y1_min = adc_val;   //update y1 analog min value
			}

			if ( adc_val > y1_max ) {
				y1_max = adc_val;   //update y1 analog max value
			}

			if ( auto_center ) { //nns: adc value correction
				adc_val = ADC_OffsetCenter ( 4096, adc_val, y1_analog_abs_params.min, y1_analog_abs_params.max,
				                             y1_offset ); //re-center adc value
				adc_val = ADC_Deadzone ( adc_val, 0x000, 0xFFF,
				                         y1_analog_abs_params.flat ); //apply flat value to adc value
			} else {
				adc_val = ADC_Deadzone ( adc_val, y1_analog_abs_params.min, y1_analog_abs_params.max,
				                         y1_analog_abs_params.flat );   //apply flat value to adc value using min and max value of the axis
			}
	...
	if ( x2_enable ) {
		if ( ads1015_enable ) {
			adc_val = ADS1015_read ( i2c_client_x1, 2 ); //ads1015
		} else {
			adc_val = i2c_smbus_read_word_swapped ( i2c_client_x2, 0 );   //mcp3021
		}
		...
		if ( adc_val >= 0 ) {
			...
			if ( adc_val < x2_min ) {
				x2_min = adc_val;   //update x2 analog min value
			}

			if ( adc_val > x2_max ) {
				x2_max = adc_val;   //update x2 analog max value
			}

			if ( auto_center ) { //nns: adc value correction
				adc_val = ADC_OffsetCenter ( 4096, adc_val, x2_analog_abs_params.min, x2_analog_abs_params.max,
				                             x2_offset ); //re-center adc value
				adc_val = ADC_Deadzone ( adc_val, 0x000, 0xFFF,
				                         x2_analog_abs_params.flat ); //apply flat value to adc value
			} else {
				adc_val = ADC_Deadzone ( adc_val, x2_analog_abs_params.min, x2_analog_abs_params.max,
				                         x2_analog_abs_params.flat );   //apply flat value to adc value using min and max value of the axis
			}
	...
	if ( y2_enable ) {
		if ( ads1015_enable ) {
			adc_val = ADS1015_read ( i2c_client_x1, 3 ); //ads1015
		} else {
			adc_val = i2c_smbus_read_word_swapped ( i2c_client_y2, 0 );   //mcp3021
		}
		...
		if ( adc_val >= 0 ) {
			...
			if ( adc_val < y2_min ) {
				y2_min = adc_val;   //update y2 analog min value
			}

			if ( adc_val > y2_max ) {
				y2_max = adc_val;   //update y2 analog max value
			}

			if ( auto_center ) { //nns: adc value correction
				adc_val = ADC_OffsetCenter ( 4096, adc_val, y2_analog_abs_params.min, y2_analog_abs_params.max,
				                             y2_offset ); //re-center adc value
				adc_val = ADC_Deadzone ( adc_val, 0x000, 0xFFF,
				                         y2_analog_abs_params.flat ); //apply flat value to adc value
			} else {
				adc_val = ADC_Deadzone ( adc_val, y2_analog_abs_params.min, y2_analog_abs_params.max,
				                         y2_analog_abs_params.flat );   //apply flat value to adc value using min and max value of the axis
			}
...
static int __init mk_setup_pad ( struct mk *mk, int idx, int pad_type_arg ) {
	...
	*/
	...
	if ( auto_center ) {
		if ( x1_enable ) {
			input_set_abs_params ( input_dev, ABS_X, 0x000, 0xFFF, x1_analog_abs_params.fuzz,
			                       x1_analog_abs_params.flat );   //nns: parameters for center offcenter values
		}

		if ( y1_enable ) {
			input_set_abs_params ( input_dev, ABS_Y, 0x000, 0xFFF, y1_analog_abs_params.fuzz,
			                       y1_analog_abs_params.flat );   //nns: parameters for center offcenter values
		}

		if ( x2_enable ) {
			input_set_abs_params ( input_dev, ABS_RX, 0x000, 0xFFF, x2_analog_abs_params.fuzz,
			                       x2_analog_abs_params.flat );   //nns: parameters for center offcenter values
		}

		if ( y2_enable ) {
			input_set_abs_params ( input_dev, ABS_RY, 0x000, 0xFFF, y2_analog_abs_params.fuzz,
			                       y2_analog_abs_params.flat );   //nns: parameters for center offcenter values
		}
	} else {
		//void input_set_abs_params(struct input_dev *dev, unsigned int axis,int min, int max, int fuzz, int flat)
		if ( x1_enable ) {
			input_set_abs_params ( input_dev, ABS_X, x1_analog_abs_params.min, x1_analog_abs_params.max,
			                       x1_analog_abs_params.fuzz, x1_analog_abs_params.flat );
		}

		if ( y1_enable ) {
			input_set_abs_params ( input_dev, ABS_Y, y1_analog_abs_params.min, y1_analog_abs_params.max,
			                       y1_analog_abs_params.fuzz, y1_analog_abs_params.flat );
		}

		if ( x2_enable ) {
			input_set_abs_params ( input_dev, ABS_RX, x2_analog_abs_params.min, x2_analog_abs_params.max,
			                       x2_analog_abs_params.fuzz, x2_analog_abs_params.flat );
		}

		if ( y2_enable ) {
			input_set_abs_params ( input_dev, ABS_RY, y2_analog_abs_params.min, y2_analog_abs_params.max,
			                       y2_analog_abs_params.fuzz, y2_analog_abs_params.flat );
		}
	}
...
static int __init mk_init ( void ) {
	...
	if ( auto_center_cfg.nargs > 0 ) { //if auto_center_analog set
		if ( auto_center_cfg.auto_center[0] ) {
			auto_center = true;   //nns: if value > 0, auto center enable
		}
	}

	if ( analog_x1_direction_cfg.nargs > 0 ) { //if x1dir set
		if ( analog_x1_direction_cfg.dir[0] < 0 ) {
			x1_reverse = true;   //nns:direction reversed
		}
	}

	if ( analog_y1_direction_cfg.nargs > 0 ) { //if y1dir set
		if ( analog_y1_direction_cfg.dir[0] < 0 ) {
			y1_reverse = true;   //nns:direction reversed
		}
	}

	if ( analog_x2_direction_cfg.nargs > 0 ) { //if x2dir set
		if ( analog_x2_direction_cfg.dir[0] < 0 ) {
			x2_reverse = true;   //nns:direction reversed
		}
	}

	if ( analog_y2_direction_cfg.nargs > 0 ) { //if y2dir set
		if ( analog_y2_direction_cfg.dir[0] < 0 ) {
			y2_reverse = true;   //nns:direction reversed
		}
	}


	x1_analog_abs_params.min =
	    ABS_PARAMS_DEFAULT_X_MIN; //x1 analog default min value for input_set_abs_params
	x1_analog_abs_params.max =
	    ABS_PARAMS_DEFAULT_X_MAX; //x1 analog default max value for input_set_abs_params
	x1_analog_abs_params.fuzz =
	    ABS_PARAMS_DEFAULT_X_FUZZ; //x1 analog default fuzz value for input_set_abs_params
	x1_analog_abs_params.flat =
	    ABS_PARAMS_DEFAULT_X_FLAT; //x1 analog default flat value for input_set_abs_params

	if ( analog_x1_abs_params_cfg.nargs > 0 ) {
		x1_analog_abs_params.min =
		    analog_x1_abs_params_cfg.abs_params[0]; //if analog_x1_abs_params_cfg set, set min value

		if ( analog_x1_abs_params_cfg.nargs > 1 ) {
			x1_analog_abs_params.max = analog_x1_abs_params_cfg.abs_params[1]; //set max value

			if ( analog_x1_abs_params_cfg.nargs > 2 ) {
				x1_analog_abs_params.fuzz = analog_x1_abs_params_cfg.abs_params[2]; //set fuzz value

				if ( analog_x1_abs_params_cfg.nargs > 3 ) {
					x1_analog_abs_params.flat = analog_x1_abs_params_cfg.abs_params[3];   //set flat value
				}
			}
		}
	}

	y1_analog_abs_params.min =
	    ABS_PARAMS_DEFAULT_Y_MIN; //y1 analog default min value for input_set_abs_params
	y1_analog_abs_params.max =
	    ABS_PARAMS_DEFAULT_Y_MAX; //y1 analog default max value for input_set_abs_params
	y1_analog_abs_params.fuzz =
	    ABS_PARAMS_DEFAULT_Y_FUZZ; //y1 analog default fuzz value for input_set_abs_params
	y1_analog_abs_params.flat =
	    ABS_PARAMS_DEFAULT_Y_FLAT; //y1 analog default flat value for input_set_abs_params

	if ( analog_y1_abs_params_cfg.nargs > 0 ) {
		y1_analog_abs_params.min =
		    analog_y1_abs_params_cfg.abs_params[0]; //if analog_y1_abs_params_cfg set, set min value

		if ( analog_y1_abs_params_cfg.nargs > 1 ) {
			y1_analog_abs_params.max = analog_y1_abs_params_cfg.abs_params[1]; //set max value

			if ( analog_y1_abs_params_cfg.nargs > 2 ) {
				y1_analog_abs_params.fuzz = analog_y1_abs_params_cfg.abs_params[2]; //set fuzz value

				if ( analog_y1_abs_params_cfg.nargs > 3 ) {
					y1_analog_abs_params.flat = analog_y1_abs_params_cfg.abs_params[3];   //set flat value
				}
			}
		}
	}

	x2_analog_abs_params.min =
	    ABS_PARAMS_DEFAULT_X_MIN; //x2 analog default min value for input_set_abs_params
	x2_analog_abs_params.max =
	    ABS_PARAMS_DEFAULT_X_MAX; //x2 analog default max value for input_set_abs_params
	x2_analog_abs_params.fuzz =
	    ABS_PARAMS_DEFAULT_X_FUZZ; //x2 analog default fuzz value for input_set_abs_params
	x2_analog_abs_params.flat =
	    ABS_PARAMS_DEFAULT_X_FLAT; //x2 analog default flat value for input_set_abs_params

	if ( analog_x2_abs_params_cfg.nargs > 0 ) {
		x2_analog_abs_params.min =
		    analog_x2_abs_params_cfg.abs_params[0]; //if analog_x2_abs_params_cfg set, set min value

		if ( analog_x2_abs_params_cfg.nargs > 1 ) {
			x2_analog_abs_params.max = analog_x2_abs_params_cfg.abs_params[1]; //set max value

			if ( analog_x2_abs_params_cfg.nargs > 2 ) {
				x2_analog_abs_params.fuzz = analog_x2_abs_params_cfg.abs_params[2]; //set fuzz value

				if ( analog_x2_abs_params_cfg.nargs > 3 ) {
					x2_analog_abs_params.flat = analog_x2_abs_params_cfg.abs_params[3];   //set flat value
				}
			}
		}
	}

	y2_analog_abs_params.min =
	    ABS_PARAMS_DEFAULT_Y_MIN; //y2 analog default min value for input_set_abs_params
	y2_analog_abs_params.max =
	    ABS_PARAMS_DEFAULT_Y_MAX; //y2 analog default max value for input_set_abs_params
	y2_analog_abs_params.fuzz =
	    ABS_PARAMS_DEFAULT_Y_FUZZ; //y2 analog default fuzz value for input_set_abs_params
	y2_analog_abs_params.flat =
	    ABS_PARAMS_DEFAULT_Y_FLAT; //y2 analog default flat value for input_set_abs_params

	if ( analog_y2_abs_params_cfg.nargs > 0 ) {
		y2_analog_abs_params.min =
		    analog_y2_abs_params_cfg.abs_params[0]; //if analog_y2_abs_params_cfg set, set min value

		if ( analog_y2_abs_params_cfg.nargs > 1 ) {
			y2_analog_abs_params.max = analog_y2_abs_params_cfg.abs_params[1]; //set max value

			if ( analog_y2_abs_params_cfg.nargs > 2 ) {
				y2_analog_abs_params.fuzz = analog_y2_abs_params_cfg.abs_params[2]; //set fuzz value

				if ( analog_y2_abs_params_cfg.nargs > 3 ) {
					y2_analog_abs_params.flat = analog_y2_abs_params_cfg.abs_params[3];   //set flat value
				}
			}
		}
	}
			...
			if ( !ads1015_enable && ads1015_cfg.address[0] == 0 ) { //use MCP3021
				...
				if ( analog_x1_cfg.address[0] >= 0 && analog_x1_cfg.address[0] < 4 ) {
					...
					if ( value >= 0 ) {
						...
						if ( debug_mode ) {
							printk ( "mk_arcade_joystick_rpi: returned %i\n", value );
						}
...
static void __exit mk_exit ( void ) {
	...
	if ( ads1015_enable ) {
		i2c_unregister_device ( i2c_client_x1 );   //nns: add ads1015 support
	}
