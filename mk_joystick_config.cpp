...
void *gpio_routine ( void * ) { //GPIO input thread routine
	if ( debug ) {
		printf ( "---Debug : GPIO : Thread (%lu) started\n", gpio_thread );   //debug
	}

	// gpio_thread_rc values : 0:ok, -1:failed, -2:fail because of wiringPi

	if ( wiringPiSetupGpio() == -1 ) {
		printf ( "GPIO : wiringPi not initialized\n" );    //failed
		gpio_thread_rc = -2;
	}
	...
	for ( int gpio_pin = 0; gpio_pin < 54; gpio_pin++ ) { //initial loop to fill empty array
		...
		if ( getAlt ( gpio_pin ) == 0 ) { //pin is a input
			...
			if ( digitalRead ( gpio_pin ) ) {
				gpio_input_activelow[gpio_pin] = 0;   //assume low as activelow
			} else {
				gpio_input_activelow[gpio_pin] = 1;
			}

			gpio_input_timestamp[gpio_pin] = 0; //initialize timestamp

			if ( gpio_pin != 20 && gpio_pin != 21 ) {
				pullUpDnControl ( gpio_pin, PUD_UP );
				//pull all input high except of pin used for power slider, allow 1ms to set pullup register
				usleep ( 1000 );
			}
		}
	}
	...
	while ( gpio_thread_rc > -1 ) { //loop until fail
		for ( int gpio_pin = 0; gpio_pin < 54; gpio_pin++ ) { //initial loop to fill array with real data
			if ( getAlt ( gpio_pin ) == 0 ) { //pin is a input
				if ( ( !gpio_input_activelow[gpio_pin] && !digitalRead ( gpio_pin ) )
				        || ( gpio_input_activelow[gpio_pin] && digitalRead ( gpio_pin ) ) ) { //gpio button is pressed
					if ( gpio_input_timestamp[gpio_pin] == 0 ) { //no timestamp
						if ( debug ) {
							printf ( "---Debug : GPIO : %d : %lli : %lli\n", gpio_pin, timestamp_msec(),
							         gpio_input_timestamp[gpio_pin] );   //debug
						}

						gpio_input_timestamp[gpio_pin] = timestamp_msec(); //update msec timestamp if no pressed before
					} else {
						if ( gpio_input[gpio_pin] != 1 && button_pressed == -1
						        && ( timestamp_msec() - gpio_input_timestamp[gpio_pin] ) >
						        50 ) { //allow to trigger only once, trigger more than 50msec to avoid detect because of noise
							if ( debug ) {
								printf ( "---Debug : GPIO : %d : %lli : %lli\n", gpio_pin, timestamp_msec(),
								         gpio_input_timestamp[gpio_pin] );   //debug
							}
...
void *adc_routine ( void * ) { //ADC input thread routine
	if ( debug ) {
		printf ( "---Debug : ADC : Thread (%lu) started\n", adc_thread );   //debug
	}
	...
	if ( ( i2c_handle = open ( i2c_bus, O_RDWR ) ) < 0 ) { //failed open i2c bus
		...
		for ( loop = 72; loop <= 79; loop++ ) { //try to detect adc chip adresses
			if ( ioctl ( i2c_handle, I2C_SLAVE, loop ) < 0 ) { //try access i2c device
				if ( debug ) {
					printf ( "---Debug: Failed to access I2C device : %d (0x%2x)\n", loop, loop & 0xFF );   //debug
				}
			} else {
				if ( read ( i2c_handle, i2c_buffer, 2 ) != 2 ) { //fail to read data
					if ( debug ) {
						printf ( "---Debug: Failed to read I2C device : %d (0x%2x)\n", loop, loop & 0xFF );   //debug
					}
				} else { //success
					if ( debug ) {
						printf ( "---Debug: I2C device detected : %d (0x%2x)\n", loop, loop & 0xFF );   //debug
					}
					...
					if ( !adc_ads1015 ) {
						i2c_buffer[0] = 0x01;
						i2c_buffer[1] = 0x00;
						i2c_buffer[2] = 0x00; //reset config register
						write ( i2c_handle, i2c_buffer, 3 );
						usleep ( 1000 ); //write register and wait 1msec
						read ( i2c_handle, i2c_buffer, 2 ); //get result
						adc_val_tmp = ( i2c_buffer[0] << 8 ) | ( i2c_buffer[1] & 0xff ); //combine to int

						if ( debug ) {
							printf ( "---Debug: read : 0x%04x : ", adc_val_tmp );   //debug
						}
						...
						if ( adc_val_tmp == 0x0000 ) { //ads1015 detected
							...
							i2c_buffer[0] = 0x01;
							i2c_buffer[1] = ( 0x83 | ads1015_config_ain[0] );
							i2c_buffer[2] = ( 0xE3 ); //config register: AIN0, 3300SPS, +-4.096v FSR
		...
		/*
			 ...
			 */
		if ( adc_detected == 0 ) {
			adc_thread_rc = -2; //no chip detected
		} else {
			adc_thread_rc = 0;   //ok
		}
	...
	while ( adc_thread_rc > -1 ) { //loop until fail
		if ( adc_user && adc_calibration ) { // user selected right adc type but calibration in progress
			if ( adc_ads1015 ) {
				ADS1015_read ( adc_address_ads1015 ); //read ads1015 values
			} else if ( adc_mcp3021 ) { //mcp3021 use
				for ( loop = 0; loop < adc_address_count; loop++ ) { //read loop
					if ( adc_address[loop] != -1 ) {
						MCP3021_read ( adc_address[loop], loop );   //read mcp3021 value
					}
				}
			}

			if ( ( time ( NULL ) - scanning_start ) > 4 ) {
				adc_calibration_done = true;    //calibration done
				adc_calibration = false;
			}
		} else if ( adc_user
		            && adc_calibration_done ) { // user selected right adc type and calibration done
			if ( adc_ads1015 ) {
				ADS1015_read ( adc_address_ads1015 ); //read ads1015 values
			} else if ( adc_mcp3021 ) { //mcp3021 use
				for ( loop = 0; loop < adc_address_count; loop++ ) { //read loop
			...
			sleep ( 1 );   // standby until user selected right adc type
		}
	}

	if ( adc_thread_rc > -3 ) {
		close ( i2c_handle );   //close the handle
	}
...
void MCP3021_read ( int addr, int index ) { //function to read MCP3021 values
	...
	if ( ioctl ( i2c_handle, I2C_SLAVE, addr ) >= 0 ) { //try access i2c device
		...
		if ( adc_center[index] == -1 ) {
			adc_center[index] = adc_val_tmp;   //set center value if not set
		}

		if ( adc_min[index] == -1 ) {
			adc_min[index] = adc_val_tmp;   //set min value if not set
		}

		if ( adc_val_tmp < adc_min[index] ) {
			adc_min[index] = adc_val_tmp;   //update min value
		}

		if ( adc_max[index] == -1 ) {
			adc_max[index] = adc_val_tmp;   //set max value if not set
		}

		if ( adc_val_tmp > adc_max[index] ) {
			adc_max[index] = adc_val_tmp;   //update max value
		}
...
void ADS1015_read ( int addr ) {
	...
	if ( ioctl ( i2c_handle, I2C_SLAVE, addr ) >= 0 ) { //try access i2c device
		...
		for ( loop = 0; loop < 4; loop++ ) { //read loop
			...
			i2c_buffer[1] = ( 0x83 | ads1015_config_ain[loop] );
			i2c_buffer[2] = ( 0xE3 ); //3300SPS, +-4.096v FSR
			write ( i2c_handle, i2c_buffer, 3 ); //write config
			...
			if ( adc_center[loop] == -1 ) {
				adc_center[loop] = adc_val_tmp;   //set center value if not set
			}

			if ( adc_min[loop] == -1 ) {
				adc_min[loop] = adc_val_tmp;   //set min value if not set
			}

			if ( adc_val_tmp < adc_min[loop] ) {
				adc_min[loop] = adc_val_tmp;   //update min value
			}

			if ( adc_max[loop] == -1 ) {
				adc_max[loop] = adc_val_tmp;   //set max value if not set
			}

			if ( adc_val_tmp > adc_max[loop] ) {
				adc_max[loop] = adc_val_tmp;   //update max value
			}

			//if(debug){printf("%d : %04d (min: %04d,center: %04d,max: %04d), ",loop,adc_val_tmp,adc_min[loop],adc_center[loop],adc_max[loop]);} //debug
		}
	} else {
		return ;   //failed to access i2c device
	}
}
	...
	for ( int delta_loop = 0; delta_loop < adc_address_count; delta_loop++ ) { //loop
		if ( adc_skip[delta_loop] == 0 ) { //adc not skipped
			...
			if ( abs ( delta_current ) > abs ( delta_max ) ) { //highest delta
				...
				if ( delta_max < 0 ) {
					adc_reverse[delta_loop] = -1;   //reverse direction
				} else {
					adc_reverse[delta_loop] = 1;
				}
			}
		}
	}
...
int Wait_User_Input ( int forbidden_pin, int timeout ) { //function to report user input
	...
	while ( true ) { //loop until input detected
		if ( button_pressed != -1 ) { //user pressed a allow key
			...
			if ( button_pressed_tmp != forbidden_pin && gpio_input_enable[button_pressed_tmp] ) {
				return button_pressed_tmp;   //return gpio pin if not a forbidden pin
			}
		}
...
bool DriverLoaded() { //function to detect if driver is loaded in lsmod
	...
	if ( fread ( i2c_buffer, 1, sizeof ( i2c_buffer ), temp_filehandle ) > 0 ) {
...
int main ( int argc, char *argv[] ) { //main
	for ( int i = 1; i < argc; ++i ) { //argument to variable
		if ( strcmp ( argv[i], "-help" ) == 0 ) {
			...
			adc_maxnoise = atoi ( argv[i + 1] );
		} else if ( strcmp ( argv[i], "-adcselect" ) == 0 ) {
			adc_user = false;
	...
	*/
	...
	while ( gpio_thread_rc < 0 ) {
		sleep ( 2 );   //wait until gpio fully initialize
	}
	...
	while ( time ( NULL ) - scanning_start < 2 ) {
		button_pressed = -1;   //read for 2sec, reset gpio button
	}
	...
	if ( DriverLoaded() ) {
		printf ( "\033[91mFailed to kill the driver, exiting\033[0m\n\n" );
		return 0; //if driver still detected, exit
	} else {
		printf ( "\033[92mDriver killed with success\033[0m\n\n" );   //continue
	}
	...
	while ( retry ) {
		...
		if ( button_tmp_gpio == button_A_gpio
		        || button_tmp_gpio == button_B_gpio ) { //valid input, ask for validation
			if ( button_tmp_gpio == button_A_gpio ) {
				printf ( "\033[2K\r" );
	...
	while ( adc_thread_rc < 0 ) { //detection in progress, wait until gpio fully initialize
		if ( ( time ( NULL ) - scanning_start ) > 9 ) { //timeout
			...
			adc_ads1015 = false;
			adc_mcp3021 = false;
			adc_detected = 0; //disable both analog possible chip
	...
	if ( adc_mcp3021 || adc_ads1015 ) { //adc chip detected
		...
		if ( !adc_user ) { //prompt user to select adc chip
			...
			while ( retry ) {
				...
				if ( button_tmp_gpio == button_A_gpio
				        || button_tmp_gpio == button_B_gpio ) { //valid input, ask for validation
					if ( button_tmp_gpio == button_A_gpio ) {
						printf ( "\033[2K\rYou selected \033[1mMCP3021A\033[0m? \033[92m(A)\033[0m for \033[1mYes\033[0m, \033[92m(B)\033[0m for \033[1mNo\033[0m" );
					} else if ( button_tmp_gpio == button_B_gpio ) {
						printf ( "\033[2K\rYou selected \033[1mADS1015\033[0m? \033[92m(A)\033[0m for \033[1mYes\033[0m, \033[92m(B)\033[0m for \033[1mNo\033[0m" );
					}
					...
					if ( button_tmpbis_gpio == button_A_gpio ) { //valid input
						if ( button_tmp_gpio == button_A_gpio ) {
							printf ( "\033[2K\r\033[1mMCP3021A\033[0m will be used\n\n" );
							adc_mcp3021 = true;
							adc_ads1015 = false;
						} else if ( button_tmp_gpio == button_B_gpio ) {
							printf ( "\033[2K\r\033[1mADS1015\033[0m will be used\n\n" );
							adc_mcp3021 = false;
							adc_ads1015 = true;
						}
			...
			if ( adc_mcp3021 ) {
				printf ( "\033[1mMCP3021A\033[0m" );
			} else if ( adc_ads1015 ) {
				printf ( "\033[1mADS1015\033[0m" );
			}
		...
		for ( tmploop = 0; tmploop < adc_address_count; tmploop++ ) { //detect high noise
			if ( adc_max[tmploop] - adc_min[tmploop] > adc_maxnoise ) { //high noise in input
				...
				if ( adc_mcp3021 ) {
					...
					printf ( "AIN%d", tmploop );   //ads
				}

				printf ( " skipped\033[0m\n" );
			} else {
				if ( adc_center[tmploop] != -1 ) {
					adc_skip[tmploop] = 0;    //if center detected, enable this i2c adress or ain
					adc_detected++;
				}
			}
		}
		...
		if ( adc_detected < 2 ) { //something wrong with analog
			...
			retrymain = true;   //allow analog
		}

		for ( tmploop = 0; tmploop < adc_address_count; tmploop++ ) {
			adc_skipbackup[tmploop] = adc_skip[tmploop];   //backup adc skip
		}
		...
		while ( retrymain ) {
			...
			for ( tmploop = 0; tmploop < adc_address_count; tmploop++ ) {
				adc_skip[tmploop] = adc_skipbackup[tmploop];   //reset adc skip
			}

			for ( axisloop = 0; axisloop < 4; axisloop++ ) {
				adc_mapping[axisloop] = -1;   //reset
			}
			...
			for ( axisloop = 0; axisloop < 4; axisloop++ ) { //start axis user definition
				if ( adc_detected ) { //remaining analog to use
					...
					while ( retry ) {
						printf ( "\033[2K\rPush \033[1m%s\033[0m analog \033[1m%s\033[0m then press \033[92m(A)\033[0m, \033[92m(B)\033[0m to \033[1mSkip\033[0m",
						         str_analog_position[axisloop], str_analog_direction[axisloop] );
						button_tmp_gpio = Wait_User_Input ( -2, -1 ); //recover user input

						if ( button_tmp_gpio == button_A_gpio
						        || button_tmp_gpio == button_B_gpio ) { //valid input, ask for validation
							if ( button_tmp_gpio == button_A_gpio ) {
								printf ( "\033[2K\r\033[1m%s\033[0m analog \033[1m%s\033[0m axis detected",
								         str_analog_position[axisloop], str_analog_axis[axisloop] );

								adc_useinput = adc_highest_delta();

								if ( adc_reverse[adc_useinput] < 0 ) {
								...
								printf ( "\033[2K\r\033[1m%s\033[0m analog \033[1m%s\033[0m axis skipped",
								         str_analog_position[axisloop], str_analog_axis[axisloop] );
								adc_useinput == -1;
							}
			...
			if ( debug ) {
				printf ( "x1: %d,y1: %d,x2: %d,y2: %d\n", adc_mapping[0], adc_mapping[1], adc_mapping[2],
				         adc_mapping[3] );   //debug
			}

			for ( tmploop = 0; tmploop < adc_address_count; tmploop++ ) {
				adc_min[tmploop] = -1;    //reset min/max
				adc_max[tmploop] = -1;
			}
			...
			if ( adc_mapping[0] > -1 && adc_mapping[1] > -1 ) { //x1 and y1 defined
				...
				if ( adc_mcp3021 ) {
					printf ( "\033[1m%d (0x%2x)\033[0m", adc_address[adc_mapping[0]],
					         adc_address[adc_mapping[0]] & 0xFF );
				} else {
					printf ( "\033[1mAIN%d\033[0m", adc_mapping[0] );
				}

				printf ( " : min=\033[1m%d\033[0m  max=\033[1m%d\033[0m\n", adc_min[adc_mapping[0]],
				         adc_max[adc_mapping[0]] );

				printf ( "\033[1m%s\033[0m : ", str_analog_axis[1] );

				if ( adc_mcp3021 ) {
					printf ( "\033[1m%d (0x%2x)\033[0m", adc_address[adc_mapping[1]],
					         adc_address[adc_mapping[1]] & 0xFF );
				} else {
					printf ( "\033[1mAIN%d\033[0m", adc_mapping[1] );
				}

				printf ( " : min=\033[1m%d\033[0m  max=\033[1m%d\033[0m\n", adc_min[adc_mapping[1]],
				         adc_max[adc_mapping[1]] );
				...
				if ( adc_mapping[2] > -1 && adc_mapping[3] > -1 ) { //x2 and y2 defined
					...
					if ( adc_mcp3021 ) {
						printf ( "\033[1m%d (0x%2x)\033[0m", adc_address[adc_mapping[2]],
						         adc_address[adc_mapping[2]] & 0xFF );
					} else {
						printf ( "\033[1mAIN%d\033[0m", adc_mapping[2] );
					}

					printf ( " : min=\033[1m%d\033[0m  max=\033[1m%d\033[0m\n", adc_min[adc_mapping[2]],
					         adc_max[adc_mapping[2]] );

					printf ( "\033[1m%s\033[0m : ", str_analog_axis[3] );

					if ( adc_mcp3021 ) {
						printf ( "\033[1m%d (0x%2x)\033[0m", adc_address[adc_mapping[3]],
						         adc_address[adc_mapping[3]] & 0xFF );
					} else {
						printf ( "\033[1mAIN%d\033[0m", adc_mapping[3] );
					}

					printf ( " : min=\033[1m%d\033[0m  max=\033[1m%d\033[0m\n", adc_min[adc_mapping[3]],
					         adc_max[adc_mapping[3]] );
				} else {
					adc_mapping[2] = -1;    //to avoid joystick with only one axis
					adc_mapping[3] = -1;
				}
			} else {
				adc_mapping[0] = -1;    //to avoid joystick with only one axis
				adc_mapping[1] = -1;
			}
			...
			if ( adc_mapping[0] > -1 && adc_mapping[1] > -1 ) {
				printf ( "Does theses values look good? \033[92m(A)\033[0m for \033[1mYes\033[0m, \033[92m(B)\033[0m to restart" );
			} else {
				printf ( "\033[91mAnalog stick require at least 2 inputs\033[0m\n\033[92m(A)\033[0m to \033[1mRestart\033[0m, \033[92m(B)\033[0m to \033[1mSkip Analog part\033[0m" );
			}
			...
			while ( retry ) {
				...
				if ( button_tmp_gpio == button_A_gpio
					...
					if ( button_tmp_gpio == button_A_gpio ) {
						...
						if ( adc_mapping[0] < 0 || adc_mapping[1] < 0 ) {
							retrymain = false;
						}
					}
		...
		if ( adc_mapping[0] > -1
		        && adc_mapping[1] > -1 ) { //x1 and y1 defined, allow selection of analog auto center
			...
			while ( retry ) {
				...
				if ( button_tmp_gpio == button_A_gpio
				        || button_tmp_gpio == button_B_gpio ) { //valid input, ask for validation
					if ( button_tmp_gpio == button_A_gpio ) {
						printf ( "\033[2K\r\033[1mFeature enabled\033[0m" );
						adc_autocenter = true;
					} else if ( button_tmp_gpio == button_B_gpio ) {
						printf ( "\033[2K\r\033[1mFeature disabled\033[0m" );
						adc_autocenter = false;
					}
		...
		printf ( "\033[91mNo analog input detected, Skiping analog part\033[0m\n\n" );   //no adc chip
	}
	...
	while ( retrymain ) {
		for ( tmploop = 0; tmploop < sizeof ( button_table ) / sizeof ( button_table[0] );
		        tmploop++ ) { //input loop
			if ( strcmp ( str_button_name[tmploop], "RESERVED" ) != 0 ) {
				...
				if ( tmploop < 13 ) { //needed
					...
					printf ( "Press \033[92m[%s]\033[0m button, wait \033[1m5sec\033[0m to skip",
					...
					if ( gpio_button_link_table[tmploop] == 12 ) { //hotkey button
						...
						while ( retry ) {
							...
							if ( button_tmp_gpio == button_A_gpio
							        || button_tmp_gpio == button_B_gpio ) { //valid input, ask for validation
								if ( button_tmp_gpio == button_A_gpio ) {
									printf ( "\033[2K\rHotkey mode set to \033[1mNormal\033[0m\n" );
									button_hkmode = 1;
								} else if ( button_tmp_gpio == button_B_gpio ) {
									printf ( "\033[2K\rHotkey mode set to \033[1mToggle\033[0m\n" );
									button_hkmode = 2;
								}
		...
		for ( tmploop = 0; tmploop < 54; tmploop++ ) {
			gpio_input_enable[tmploop] = true;   //reset forbidden gpio
		}
	...
	sprintf ( text_config_buffer, " hkmode=%d", button_hkmode );
	strcat ( text_config, text_config_buffer ); //hotkey

	if ( adc_mapping[0] > -1 && adc_mapping[1] > -1 ) {
		sprintf ( text_config_buffer, " i2cbus=%d", 1 );
		strcat ( text_config, text_config_buffer ); //i2cbus

		if ( adc_autocenter ) {
			sprintf ( text_config_buffer, " auto_center_analog=%d", 1 );    //auto_center_analog
			strcat ( text_config, text_config_buffer );
		}

		if ( adc_ads1015 ) {
			sprintf ( text_config_buffer, " ads1015addr=%d", adc_address_ads1015 );    //ads1015addr
			strcat ( text_config, text_config_buffer );
		}

		if ( adc_mapping[0] > -1 && adc_mapping[1] > -1 ) { //x1 and y1 defined
			if ( adc_mcp3021 ) {
				sprintf ( text_config_buffer, " x1addr=%d y1addr=%d", adc_address[adc_mapping[0]],
				          adc_address[adc_mapping[1]] );
				strcat ( text_config, text_config_buffer ); //x1addr, y1addr for mcp3021
			} else {
				sprintf ( text_config_buffer, " x1addr=%d y1addr=%d", adc_mapping[0], adc_mapping[1] );
				//x1addr, y1addr for ads1015
				strcat ( text_config, text_config_buffer );
			}

			if ( adc_reverse[adc_mapping[0]] < 1 ) {
				sprintf ( text_config_buffer, " x1dir=%d", -1 );
				strcat ( text_config, text_config_buffer ); //x1dir
				sprintf ( text_config_buffer, " x1params=%d,%d,16,300", 4096 - adc_min[adc_mapping[0]],
				          4096 - adc_max[adc_mapping[0]] );
				strcat ( text_config, text_config_buffer ); //x1params
			} else {
				sprintf ( text_config_buffer, " x1params=%d,%d,16,300", adc_min[adc_mapping[0]],
				          adc_max[adc_mapping[0]] );
				strcat ( text_config, text_config_buffer ); //x1params
			}

			if ( adc_reverse[adc_mapping[1]] < 1 ) {
				sprintf ( text_config_buffer, " y1dir=%d", -1 );
				strcat ( text_config, text_config_buffer ); //y1dir
				sprintf ( text_config_buffer, " y1params=%d,%d,16,300", 4096 - adc_min[adc_mapping[1]],
				          4096 - adc_max[adc_mapping[1]] );
				strcat ( text_config, text_config_buffer ); //y1params
			} else {
				sprintf ( text_config_buffer, " y1params=%d,%d,16,300", adc_min[adc_mapping[1]],
				          adc_max[adc_mapping[1]] );
				strcat ( text_config, text_config_buffer ); //y1params
			}
		}
		...
		if ( adc_mapping[2] > -1 && adc_mapping[3] > -1 ) { //x2 and y2 defined
			if ( adc_mcp3021 ) {
				sprintf ( text_config_buffer, " x2addr=%d y2addr=%d", adc_address[adc_mapping[2]],
				          adc_address[adc_mapping[3]] );
				strcat ( text_config, text_config_buffer ); //x2addr, y2addr for mcp3021
			} else {
				sprintf ( text_config_buffer, " x2addr=%d y2addr=%d", adc_mapping[2], adc_mapping[3] );
				//x2addr, y2addr for ads1015
				strcat ( text_config, text_config_buffer );
			}

			if ( adc_reverse[adc_mapping[2]] < 1 ) {
				sprintf ( text_config_buffer, " x2dir=%d", -1 );
				strcat ( text_config, text_config_buffer ); //x2dir
				sprintf ( text_config_buffer, " x2params=%d,%d,16,300", 4096 - adc_min[adc_mapping[2]],
				          4096 - adc_max[adc_mapping[2]] );
				strcat ( text_config, text_config_buffer ); //x2params
			} else {
				sprintf ( text_config_buffer, " x2params=%d,%d,16,300", adc_min[adc_mapping[2]],
				          adc_max[adc_mapping[2]] );
				strcat ( text_config, text_config_buffer ); //x2params
			}

			if ( adc_reverse[adc_mapping[3]] < 1 ) {
				sprintf ( text_config_buffer, " y2dir=%d", -1 );
				strcat ( text_config, text_config_buffer ); //y2dir
				sprintf ( text_config_buffer, " y2params=%d,%d,16,300", 4096 - adc_min[adc_mapping[3]],
				          4096 - adc_max[adc_mapping[3]] );
				strcat ( text_config, text_config_buffer ); //y2params
			} else {
				sprintf ( text_config_buffer, " y2params=%d,%d,16,300", adc_min[adc_mapping[3]],
				          adc_max[adc_mapping[3]] );
				strcat ( text_config, text_config_buffer ); //y2params
			}
		}
	...
	sprintf ( text_config_buffer, " gpio=" );
	strcat ( text_config, text_config_buffer ); //gpio

	for ( tmploop = 0;
	        tmploop < sizeof ( gpio_button_link_table ) / sizeof ( gpio_button_link_table[0] );
	        tmploop++ ) { //gpio
		if ( tmploop != 0 ) {
			sprintf ( text_config_buffer, "," );    //add comma
			strcat ( text_config, text_config_buffer );
		}

		sprintf ( text_config_buffer, "%d", button_table[tmploop]*button_table_logic[tmploop] );
		strcat ( text_config, text_config_buffer ); //gpio pin
	}
	...
	while ( retry ) {
		...
		if ( button_tmp_gpio == button_A_gpio
		        || button_tmp_gpio == button_B_gpio ) { //valid input, ask for validation
			if ( button_tmp_gpio == button_A_gpio ) {
				printf ( "\033[2K\rAre you really sure? \033[92m(A)\033[0m for \033[1mYes\033[0m, \033[92m(B)\033[0m for \033[1mNo\033[0m" );
			} else if ( button_tmp_gpio == button_B_gpio ) {
				printf ( "\033[2K\rExit without saving? \033[92m(A)\033[0m for \033[1mYes\033[0m, \033[92m(B)\033[0m for \033[1mNo\033[0m" );
			}
	...
	if ( save_config ) {
		if ( access ( config_path, R_OK ) != 0 ) {
			...
			sprintf ( config_backup_path, "/etc/modprobe.d/mk_arcade_joystick-%d.bak",
			          localtime ( &file_attribute.st_mtime ) ); //create backup full path
		...
		printf ( "\033[1mExit without modifications\033[0m\n" );
	}
